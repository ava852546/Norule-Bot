...
public final class WelcomePreviewService {
    private final WebControlServer owner;

    public WelcomePreviewService(WebControlServer owner) {
        this.owner = owner;
    }

    public void handleWelcomePreview(HttpExchange exchange, Guild guild, Member member) throws IOException {
        ...
        var welcome = owner.settingsService().getWelcome(guild.getIdLong());
        ...
        String welcomeTitle = welcome.getTitle() == null || welcome.getTitle().isBlank()
                ? owner.i18n().t(lang, "welcome.default_title")
                : welcome.getTitle();
        String welcomeMessage = welcome.getMessage() == null || welcome.getMessage().isBlank()
                ? owner.i18n().t(lang, "welcome.default_message")
                : welcome.getMessage();
        String title = owner.formatWelcomeTemplate(welcomeTitle, user, guild);
        String message = owner.formatWelcomeTemplate(welcomeMessage, user, guild);
        if (message.isBlank()) {
